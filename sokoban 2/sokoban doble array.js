...
var game = {
    ...
    createPlayer: function(){
      ...
      console.log(this.newMap.map[player.data("y")][player.data("x")]);
    ...
    movePerson:function(player,obj){
       ...
       console.log(this.newMap.map[player.data('y')])
        if(this.newMap.map[player.data('y')+yOffset][player.data('x')+xOffset] != 6 ){
        ...
        if(that.touchBox(player,$(value)) && (that.newMap.map[player.data('y')+yOffset][player.data('x')+xOffset] != 6))//no need to display true
            {        
